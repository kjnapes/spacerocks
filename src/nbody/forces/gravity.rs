...
//         let n_entities = entities.len();
//         let inv.mass()es: Vec<f64> = entities.iter().map(|e| 1.0 / e.mass()).collect();
...
//             let.mass()_i = entities[i].mass();

//             for j in (i + 1)..n_entities {
//                 let pos_j = &entities[j].position;
//                 let.mass()_j = entities[j].mass();
...
//                     let force = GRAVITATIONAL_CONSTANT *.mass()_i *.mass()_j * inv_dist_cube;

//                     let fx = force * dx;
//                     let fy = force * dy;
//                     let fz = force * dz;

//                     atomic_add(&acc_i[0], -fx * inv.mass()es[i]);
//                     atomic_add(&acc_i[1], -fy * inv.mass()es[i]);
//                     atomic_add(&acc_i[2], -fz * inv.mass()es[i]);

//                     atomic_add(&accelerations[j][0], fx * inv.mass()es[j]);
//                     atomic_add(&accelerations[j][1], fy * inv.mass()es[j]);
//                     atomic_add(&accelerations[j][2], fz * inv.mass()es[j]);
//                 }
//             }
//         });
