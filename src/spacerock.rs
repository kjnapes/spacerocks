...
impl SpaceRock {
    ...
    pub fn from_spice(name: &str, epoch: &Time, frame: &ReferencePlane, origin: &Origin) -> Self {
    ...
    pub fn from_xyz(name: &str, x: f64, y: f64, z: f64, vx: f64, vy: f64, vz: f64, epoch: Time, frame: &ReferencePlane, origin: &Origin) -> Self {
    ...
    // pub fn from_state(name: &str, state: StateVector, epoch: Time, frame: &ReferencePlane, origin: &Origin) -> Self {
    ...
    // pub fn from_kepler(name: &str, orbit: KeplerOrbit, epoch: Time, frame: &ReferencePlane, origin: &Origin) -> Self {
    ...
    ///
    /// let epoch = Time::now();
    /// let rock = SpaceRock::from_horizons("Arrokoth", &epoch, &ReferencePlane::J2000, &Origin::SSB);
    /// ```
    pub fn from_horizons(name: &str, epoch: &Time, frame: &str, origin: &str) -> Result<Self, Box<dyn std::error::Error>> {

        let frame = ReferencePlane::from_str(frame)?;
    ...
    pub fn from_spherical(name: &str, phi: f64, theta: f64, r: f64, vr: f64, vo: f64, psi: f64, epoch: Time, frame: &ReferencePlane, origin: &Origin) -> Self {
